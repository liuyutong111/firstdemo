    ...
    <el-container>
      ...
      <div class="kz">
        ...
        <el-col :span="3">
          ...
          <div class="grid-content bg-purple-light bjheadr_tjbj qy_add" @click="add_xz(xzData.parentId)" v-if="flag==2">添加小组</div>
        </el-col>
      </div>
      <section class="bj_container">
        ...
        <div class="bb" v-if="flag==1">
          <div class="currert">
            ...
            <span class="title">{{ bmData.qyName }}</span>
          ...
          <div v-for="(item,index) in bmData.child" :key="index" class="general">
            ...
            <div class="operation"> 
              ...
              <el-button type="text" size="small" @click="goToxz(item.groupId)"><i class="el-icon-s-unfold" style="font-size:20px;"></i></el-button>
            </div>
          </div>
        </div>
        <!-- 小组 -->
        <div class="cc" v-if="flag==2">
          <div class="current1 currert">
            <span style="opacity: 0;">空的</span>
            <span class="title">{{ xzData.qyName}}|{{ xzData.bmName }}</span>
            <el-button type="text" size="small" @click="back_bm"><i class="el-icon-thumb" style="font-size:20px;color:#ccc;"></i></el-button>
          </div>
          <div v-for="(item,index) in xzData.child" :key="index" class="general">
            <span>{{ item.groupName }}</span>
            <div class="operation"> 
              <el-button type="text" size="small" @click="update_xz(item.groupName, item.groupId)"><i class="el-icon-edit" style="font-size:20px;"></i></el-button>
              <el-button type="text" size="small" @click="del_xz(item.groupId)"><i class="el-icon-delete" style="font-size:20px;"></i></el-button>
            </div>
          </div>
        </div>
      </section>
  ...
  methods: {
    ...
    add_xz(id){
      ...
      this.$prompt('添加小组', '提示', {
        ...
        // console.log(value);
        this.groupName = value;
        // __form 添加部门时的FormData对象
        let __form = new FormData();
        __form.append("groupName", this.groupName);
        __form.append("remarks", '3级分类');
        __form.append("parentId", id);
        __form.append("indexId", "3");
        __form.append("userToken", this.token);
        console.log(__form);
        groupadd(__form).then(res =>{
          this.reload();
          // console.log(res);
        }).catch(err =>{
          console.log('请求出错', err);
    ...
    goToxz(id){
      ...
      // console.log(id);
      this.flag = 2;
      let xzAlldata = this.bmData;
      let qyName = this.bmData.qyName;
      // console.log(xzAlldata.child[0].groupId);
      for(var i=0; i<xzAlldata.child.length; i++){
        if(id == xzAlldata.child[i].groupId){
          let bmName = xzAlldata.child[i].groupName;
          let parentId = xzAlldata.child[i].groupId;
          let msg = {
            qyName: qyName,
            bmName: bmName,
            parentId: parentId,
            child: xzAlldata.child[i].child
          }
          // console.log(msg);
          this.xzData = msg
          break;
